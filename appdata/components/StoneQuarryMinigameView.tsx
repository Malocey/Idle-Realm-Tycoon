...
import MinigameCell from './StoneQuarryMinigameView/MinigameCell';
import MinigameParticipantDisplay from './StoneQuarryMinigameView/MinigameParticipantDisplay';
import MinigameUpgradesSection, { UpgradeConfig } from './StoneQuarryMinigameView/MinigameUpgradesSection'; // UpgradeConfig might be better in a shared types if used by more parent components.


const CELL_SIZE_PX_FOR_POPUP_OFFSET = 40;

interface AnimatedPopupData extends MinigameResourcePopupEvent {
...
const StoneQuarryMinigameView: React.FC = () => {
  ...
  const [animatedPopups, setAnimatedPopups] = useState<AnimatedPopupData[]>([]);
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    if (minigameState && minigameState.popupEvents && minigameState.popupEvents.length > 0) {
      const newPopupsToAnimate: AnimatedPopupData[] = [];
      minigameState.popupEvents.forEach(event => {
        ...
        newPopupsToAnimate.push({ ...event, animationStyle, text, topStyle, leftStyle });
      });
          ...
          <div className="absolute inset-0 pointer-events-none overflow-hidden">
            {animatedPopups.map(popup => (
              <div
                ...
                style={{
                  top: popup.topStyle,
                  left: popup.leftStyle,
                  color: RESOURCE_COLORS[popup.resourceType] || 'white',
                  ...popup.animationStyle,
                }}
              >
                {popup.text}
              </div>
